...
int main(int argc, char *argv[])
{
	...
	int res;
	char sql_cmd[200] = { 0 };
	MYSQL_INFO mysql_info = {"localhost","woojin","woojin5020","chat_server_id_passwd"};
	int login_flag = 0;
	...
	// MySQL 서버 접속 (DB 이름 없이)
    if (mysql_real_connect(con, mysql_info.host, mysql_info.user, mysql_info.pass,
		mysql_info.dbname, 0, NULL, 0) == NULL)
	{
	...
	// 2. DB 없으면 생성
	if (mysql_query(con, "CREATE DATABASE IF NOT EXISTS chat_server_id_passwd")) {
		fprintf(stderr, "%s\n", mysql_error(con));
		exit(1);
	}

	// 3. 방금 만든 DB 사용
	if (mysql_select_db(con, "chat_server_id_passwd")) {
		fprintf(stderr, "%s\n", mysql_error(con));
		exit(1);
	}
	fputs("DB connected!\n",stdout);
	...
	// // CLIENT_INFO client_info[MAX_CLNT] = {{0,-1,"","1","PASSWD"} //원본
	// // id와 pw는 10바이트 공간으로 할당되어 있는데 파일이 그 이상이 들어 있으면
	// // 오버플로우가 발생할 수 있음.
	// // 그래서 유효성 검사를 해주는게 좋다.(= id, pw가 10byte 이하인지 검사)
	// while(fscanf(in, "%s %s", (client_info+i)->id, (client_info+i)->pw) > 0)
	// {
	...
	while(1) 
	{
		...
		while(1)
		{
			...
			else msg[str_len-1] = '\0';

			printf("len : %d, str : %s\n",str_len, msg);

			if(!strcmp(msg,"1"))
			{
				printf("hello sign in\n");
				break;
			}
			else if(!strcmp(msg,"2"))
			{
				printf("hello sign up\n");
				break;
			}
		}
		//printf("end\n");
