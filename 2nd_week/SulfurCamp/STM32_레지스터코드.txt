...
//    return 0;
//}
/* EXTI 설정: PA10 Falling Edge */
void Setup_EXTI_PA10(void)
{
    // SYSCFG 클럭 On (APB2ENR bit14)
    RCC->APB2ENR |= (1 << 14);

    // EXTI10을 Port A에 매핑 (EXTICR[2]의 [3:0] = 0000)
    SYSCFG->EXTICR[2] &= ~(0xF << 0);

    // 인터럽트 마스크/엣지 설정
    EXTI->IMR  |=  (1 << 10);  // 라인10 인터럽트 허용
    EXTI->RTSR &= ~(1 << 10);  // Rising 비활성
    EXTI->FTSR |=  (1 << 10);  // Falling 활성 (풀이업에서 눌림=Low)
...
{
    if (EXTI->PR & (1 << 10)) {
        EXTI->PR = (1 << 10);  // W1C: pending 클리어

        // 간단 디바운스 (원치 않으면 제거)
        for (volatile int i = 0; i < 20000; i++); // 수백 us 정도
        if ((GPIOA->IDR & (1 << 10)) == 0) {     // 여전히 Low면 유효 눌림
            g_button_irq = 1;
        }
    }
}
